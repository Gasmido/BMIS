...
function positionCheck($connn, $com, $position, $name) { 
    ...
    if ($row = mysqli_fetch_assoc($resultdata)) {
        // Check if the name matches the one in the existing row
        if ($row['name'] === $name) {
            // Name matches, so we return false
            return false;
        } else {
            // Committee and position are found, but the name does not match
            return true;
        }
    } else {
        // No row found with the same committee and position
        return true;
    }
}
...
function createComplaint($conn, $complaint_subject, $complaint_details, $userid, $useremail, $datetoday) {
    // First, check if the user exists in the 'users' table
    $checkQuery = "SELECT user_id, complaint_count, last_complaint_date FROM users WHERE user_id = ?";
    $checkStmt = $conn->prepare($checkQuery);
    $checkStmt->bind_param("i", $userid); // "i" for integer
    $checkStmt->execute();
    $checkStmt->store_result();

    // If the user doesn't exist, exit and return an error
    if ($checkStmt->num_rows === 0) {
        echo "$userid User does not exist.";
        $checkStmt->close();
        return;
    }

    // Fetch user data
    $checkStmt->bind_result($userId, $complaintCount, $lastComplaintDate);
    $checkStmt->fetch();

    // Check if 24 hours have passed since the last complaint
    if (strtotime($lastComplaintDate) < strtotime('-1 day')) {
        // If 24 hours have passed, reset the complaint_count to 0
        $resetQuery = "UPDATE users SET complaint_count = 0 WHERE user_id = ?";
        $resetStmt = $conn->prepare($resetQuery);
        $resetStmt->bind_param("i", $userid);
        $resetStmt->execute();
        $resetStmt->close();
        
        // Reset the complaint count after 24 hours
        $complaintCount = 0;
    }

    // If the complaint count is 5 or more and 24 hours have not passed
    if ($complaintCount >= 5 && strtotime($lastComplaintDate) > strtotime('-1 day')) {
        $_SESSION['bad'] = "Sorry! You have reached the 5 complaints limit per 24 hours!";
        $checkStmt->close();
        header("Location: complaintt");
        exit();
    }

    // Proceed with inserting the complaint into the complaints table
    $query = "INSERT INTO complaints (complaint_subject, complaint_details, userid, useremail, datetoday)
              VALUES (?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("ssiss", $complaint_subject, $complaint_details, $userid, $useremail, $datetoday);
    
    // Execute the query
    if ($stmt->execute()) {
        // Update the user's complaint count and last complaint date
        $updateQuery = "UPDATE users SET complaint_count = complaint_count + 1, last_complaint_date = ? WHERE user_id = ?";
        $updateStmt = $conn->prepare($updateQuery);
        $updateStmt->bind_param("si", $datetoday, $userid);
        $updateStmt->execute();
        
        session_start();
        $_SESSION['good'] = "Complaint Submitted Successfully!";
        header("Location: complaintt"); // Redirect on success
        exit();
    } else {
        echo "Error: " . $stmt->error; // Output error message if insert fails
    }
