							...
							<?php
								if (isset($_GET["error"])) {
									if ($_GET["error"] == "Empty_input") {
										...
										echo "<p class='example'>Password length shouldn't be less than 8</p>";
									}
										...
										echo "<p class='example' style='font-size:12px'>Password must include at least 1 uppercase letter, 1 lowercase letter, 1 number, and 1 special character!</p>";
									}
			...
			<form action="sign_process.php" method="POST">
				<div class="inputsr">
					...
					<p style="display:inline;">Password:</p><label style="color:red;"> *</label><br>
						<input id="txt2" type="password" name="pass" onkeyup="saveValue(this);" placeholder="Enter your password" maxlength="60" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$" title="Password must have at minimum 8 characters and 16 at maximum and should include at least 1 uppercase letter, 1 lowercase letter, 1 number, and 1 special character!"><br>
