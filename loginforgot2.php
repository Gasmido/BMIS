...
$secretKey = '6LdC5r4pAAAAAI57LWy8yH6YVV_ndKLzuQefGXUO'; 
	...
	require_once 'func.php';
	 if (empInpEmail($users) !== false) {
				header("location: loginforgot.php?error=Empty_input");
				exit();
			}
			if (invalidUname($users) !== false) {
				header("location: loginforgot.php?error=Invalid_E-mail");
				exit();
			}
			if (NoPass($connn, $users) !== false) {
				header("location: loginforgot.php?error=Nopass");
				exit();
			}
			if (duplicateEmail($connn, $users) !== false) {
